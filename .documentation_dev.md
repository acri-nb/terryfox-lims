...
- **gunicorn_start.sh**: Script to start Gunicorn WSGI server
  - Configures workers, binding, and logging
  - Collects static files automatically

- **start_production.sh**: Main script to start the application in production
  - Kills any existing Gunicorn processes
...
- **terryfox_nginx.conf**: Nginx configuration template
  - Proxies to Gunicorn
...
- **HTTPS.md**: Documentation for HTTPS configuration
  - Instructions for domain-based and IP-based HTTPS
  - Self-signed certificate generation
  - Browser configuration for self-signed certificates

- **setup_https_ip.sh**: Script to automate HTTPS setup for IP-based access
...
2. **Collect Static Files**:
   ```bash
   python manage.py collectstatic --settings=terryfox_lims.settings_prod --noinput
   ```
...
For a more robust setup with Nginx, refer to PRODUCTION.md and HTTPS.md.
...
3. Review and enable the security settings in settings_prod.py when using HTTPS
4. When using a self-signed certificate for HTTPS, users will need to accept the certificate in their browsers
