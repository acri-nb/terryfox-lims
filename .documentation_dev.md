...
- Web server (Nginx or Apache, optional, not required for direct HTTPS)
- WSGI server (Gunicorn, installed in requirements.txt)
- Additional packages for HTTPS: django-extensions, werkzeug, pyOpenSSL

### Production Files

The following files have been created to facilitate production deployment:

- **settings_prod.py**: Production-specific Django settings
  - DEBUG = False
  - Secure settings for cookies and headers
  - WhiteNoise configuration for static files
  - Environment variable configuration via python-decouple
  - django_extensions added to INSTALLED_APPS for HTTPS support

- **.env**: Environment variables for production
  - SECRET_KEY
  - ALLOWED_HOSTS (includes localhost, 127.0.0.1, and 192.168.7.13)
  - Email configuration
  - HTTPS configuration settings (SECURE_SSL_REDIRECT=True, etc.)

- **gunicorn_start.sh**: Script to start the application with HTTPS support
  - Modified to support SSL certificates
  - Uses Django's runserver_plus for direct SSL handling
  - Collects static files automatically
  - Performs certificate validation

- **start_production.sh**: Main script to start the application in production
  - Kills any existing server processes
  - Activates the Conda environment directly
  - Verifies environment and requirements
  - Generates self-signed SSL certificates if needed
  - Starts Django with HTTPS on port 8443
  - Displays access information with HTTPS URLs

- **check_production.py**: Script to validate production settings
  - Checks security settings
  - Validates static files
  - Ensures all required packages are installed

- **terryfox_nginx.conf**: Nginx configuration template (optional)
  - Proxies to Gunicorn
  - Serves static files efficiently
  - Can be configured for HTTPS if Nginx is used

- **PRODUCTION.md**: Documentation for production deployment
  - Step-by-step instructions
  - Multiple HTTPS options including direct Django SSL
  - Troubleshooting tips for HTTPS
  - Advanced configuration options

- **setup_https_ip.sh**: Script to automate HTTPS setup for IP-based access with Nginx
  - Generates a self-signed certificate with IP SANs
  - Configures Nginx for HTTPS (requires Nginx to be installed)
  - Sets appropriate permissions and security headers
...
3. **Start the Application with HTTPS**:
   ...
   - Start Django with HTTPS support on port 8443

4. **(Alternative) Enable HTTPS with Nginx**:
   ```bash
   # Only if you have Nginx installed and prefer to use it as a reverse proxy
   sudo ./setup_https_ip.sh
   ```

The application will be available at `https://192.168.7.13:8443` with the current configuration.

### Important Security Considerations

1. The SECRET_KEY should be kept secure and not committed to source control
2. Always use HTTPS in a public-facing production environment
3. The security settings in settings_prod.py are already configured for HTTPS (SECURE_SSL_REDIRECT=True, etc.)
4. The self-signed certificates are stored in the user's home directory (~ssl/) and automatically generated if missing
5. When accessing the application with a self-signed certificate, users will need to accept the security warning in their browsers
6. While the current configuration with Django's runserver_plus is functional, it is designed for development/testing and smaller deployments
7. For high-traffic production environments, consider using a more robust setup with Nginx as a reverse proxy
