...
2. **Database Configuration**:
   ...
   CREATE DATABASE terryfox_lims;
   CREATE USER terryfox_user WITH PASSWORD 'secure_password';
   GRANT ALL PRIVILEGES ON DATABASE terryfox_lims TO terryfox_user;
   ```
...
4. **Django Configuration for Production**:
   - Create an `.env` file for environment variables
   - Configure `settings.py` to use production settings

5. **Running Migrations and Collecting Static Files**:
   ```bash
...
6. **Gunicorn Configuration**:
   Create a systemd file to manage the Gunicorn service.

7. **Nginx Configuration**:
   Configure Nginx to serve static files and act as a proxy to Gunicorn.

8. **HTTPS Configuration**:
   Use Let's Encrypt to obtain an SSL certificate and configure HTTPS.
