...
### HTTPS: two servers answer to one name

`candig-lims.cair.mun.ca` resolves to **134.153.80.9**, which is not this machine. That is a MUN
proxy, and it holds a **valid Sectigo certificate** for the name, good until January 2027. It
forwards to this VM and serves the application correctly.

The VM's own certificate, at `/root/ssl/terryfox.crt`, is self-signed. Users on the internal
network reach the VM directly rather than through the proxy, so they are the ones who see
*"Not secure"*. It had also been **expired for 77 days** when this was noticed, with a CN of
`10.220.115.67` — an address the machine no longer has — and no mention of
`candig-lims.cair.mun.ca` at all.

The cause is in `gunicorn_start_robust.sh`: it generates the certificate only when the file is
**missing**, never when it is expired. Nothing else looked at it either. `ops/status.sh` and
`watchdog.sh` now both check expiry and warn 30 days ahead.

`sudo ./ops/renew_self_signed.sh` regenerates it with the right names and a long validity. It
**refuses to run when the installed certificate was signed by an authority** — comparing subject
to issuer — because overwriting a trusted certificate with a self-signed one would be a silent
regression that puts the browser warning back.

Let's Encrypt is not an option here, and this is verified rather than assumed: port 80 does not
listen on the VM, and an ACME HTTP-01 challenge sent to the public name returns 301 from the MUN
proxy, so it never reaches this machine. `setup_letsencrypt.sh` at the repository root belongs to
an architecture that is no longer in use.

The real fix is a copy of the Sectigo certificate and key, installed in `/root/ssl/`. Only MUN
can provide it; a self-signed certificate, however well formed, is never trusted by a browser.
